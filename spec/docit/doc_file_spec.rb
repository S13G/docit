...
RSpec.describe Docit::DocFile do
  before do
  ...
  describe "use_docs in controller" do
    ...
    it "registers all docs from the module onto the controller" do
      ...
      index_op = Docit::Registry.find(controller: "UsersController", action: "index")
      expect(index_op._summary).to eq("List users")
      expect(index_op._tags).to eq(["Users"])

      create_op = Docit::Registry.find(controller: "UsersController", action: "create")
    ...
    it "can mix use_docs with inline swagger_doc" do
      ...
      expect(Docit::Registry.operations.length).to eq(3)
      destroy_op = Docit::Registry.find(controller: "UsersController", action: "destroy")
